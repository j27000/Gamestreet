            ...
            <nav className="bg-white dark:bg-gray-900">
                <div className="border-b border-slate-200 dark:border-b-0 flex flex-wrap justify-between items-center mx-auto max-w-screen-xl px-4 md:px-6 py-3">
                    <Link to="/" className="flex items-center">
                        <img src={logo} className="mr-4 h-12" alt="CodeBook Logo" />
                        <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">GameStreet</span>
                    </Link>
                    <div className="flex items-center relative">
                        ...
                        {drop && (token ? <DropdownLoggedIn notdrop={notdrop} /> : <DropdownLoggedOut notdrop={notdrop} />)}
                    </div>
                </div>
